...
- `when()`: Receive a condition (To learn more about the conditions, please go to the [`KConditions`](/docs/conditions/eq) section).
- `then()`: Receive a column or a value.
...
- `val(java.lang.String)`: Allow to introduce a `String` value.
- `val(java.lang.Number)`: Allow to introduce a `Number` value.
- `val(java.time.LocalDate)`: Allow to introduce a `LocalDate` value.
- `val(java.time.LocalDateTime)`: Allow to introduce a `LocalDateTime` value.
- `val(java.util.UUID)`: Allow to introduce a `UUID` value.
...
```sql showLineNumbers
SELECT COALESCE(au.first_name, ?, au.last_name), CAST(au.id AS TEXT), TO_CHAR(au.created_at, ?)
FROM app_user au
```
